...
export default function Dashboard({ token, onLogout }: Props) {
  ...
  const api = (path: string, opts?: any) => fetch(`/api${path}`, {
    ...opts, headers: { ...opts?.headers, 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
  }).then(r => r.json());

  useEffect(() => { api('/dashboard/stats').then(setStats); api('/orgs').then(setOrg); api('/billing/subscription').then(setSub); api('/billing/plans').then(setPlans); }, [token]);
      ...
      <header style={{ background: '#131c31', borderBottom: '1px solid #1e293b', padding: '0 24px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
            ...
            {tabs.map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ background: 'none', border: 'none', color: activeTab === t.id ? '#6366f1' : '#94a3b8', cursor: 'pointer', fontWeight: 500, fontSize: '.9rem', padding: '20px 0', borderBottom: activeTab === t.id ? '2px solid #6366f1' : '2px solid transparent' }}><i className={`fas ${t.icon}`}></i> {t.label}</button>
            ))}
          </div>
        ...
        {activeTab === 'overview' && stats && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(250px,1fr))', gap: 20, marginBottom: 32 }}>
              ...
              <StatCard title="Active Projects" value={stats.activeProjects.current} change={stats.activeProjects.change} icon="fa-folder" color="#06b6d4" />
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={stats.usersByPlan} dataKey="count" nameKey="plan" cx="50%" cy="50%" outerRadius={80} label={({ plan, percent }) => `${plan} ${(percent * 100).toFixed(0)}%`}>
                      {stats.usersByPlan.map((_: any, i: number) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                    </Pie>
                    <Tooltip contentStyle={{ background: '#0b1120', border: '1px solid #1e293b', borderRadius: 8 }} />
                  </PieChart>
                </ResponsiveContainer>
          ...
          <div>
            ...
            <div style={{ background: '#131c31', border: '1px solid #1e293b', borderRadius: 16, padding: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                ...
                <button style={{ padding: '10px 20px', borderRadius: 10, background: '#6366f1', color: '#fff', fontWeight: 600, fontSize: '.85rem', cursor: 'pointer', border: 'none' }}><i className="fas fa-plus"></i> Invite</button>
              </div>
